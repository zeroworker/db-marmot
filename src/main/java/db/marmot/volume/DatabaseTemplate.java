...
public class DatabaseTemplate implements DataSourceTemplate {
	...
	public List<DataTable> getDataTables() {
		return jdbcTemplate.execute(new ConnectionCallback<List<DataTable>>() {

			public List<DataTable> doInConnection(Connection con) throws SQLException, DataAccessException {
				List<DataTable> dataSourceTables = Lists.newArrayList();
				...
				try {
					while (rs.next()) {
						 ...
						 */
						String content = rs.getString("REMARKS");
						dataSourceTables.add(new DataTable(tableName, content));
					}
				} finally {
					JdbcUtils.close(rs);
				}
	 ...
	 * @return 数据字段
	 */
	public List<DataColumn> getDataColumns(String dbName,long volumeId, String sqlScript) {
		...
		return getJdbcTemplate(dbName).query(sqlSelectBuilder.toString(), new ResultSetExtractor<List<DataColumn>>() {
			...
			public List<DataColumn> extractData(ResultSet rs) throws SQLException, DataAccessException {
				...
				for (int index = 1; index <= columnCount; index++) {
					...
					//-构建数据集数据字段
					DataColumn dataColumn = new DataColumn(volumeId);
