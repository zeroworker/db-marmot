 ...
 * @author shaokang
 */
public class LessEqualOperatorsConverter implements OperatorsConverter {
	...
	public void validateValue(ColumnType columnType, Object value) {
		if (columnType != ColumnType.number && columnType != ColumnType.date) {
			throw new ConverterException("小于等于运算符字段类型只支持数字和时间");
		}
		if (!(value instanceof Number) && !(value instanceof Date)) {
			throw new ConverterException("小于等于运算符比较值只支持数字和时间");
		}
		if (columnType == ColumnType.number && !(value instanceof Number)) {
			throw new ConverterException("小于等于运算符字段类型为数字,比较值必须为数字类型");
		}
		if (columnType == ColumnType.date && !(value instanceof Date)) {
			throw new ConverterException("小于等于运算符字段类型为时间,比较值必须为时间");
		}
	}
