 ...
 * @author shaokang
 */
public class InOperatorsConverter implements OperatorsConverter {
	...
	public void validateValue(ColumnType columnType, Object value) {
		if (columnType != ColumnType.string) {
			throw new ConverterException("包含运算符只支持文本类型字段");
		}
		if (!(value instanceof String) && !(value instanceof List)) {
			throw new ConverterException("包含运算符比较值只支持文本或者文本集合");
		}
		if (value instanceof List) {
			Object obj = ((List) value).get(0);
			if (!(obj instanceof String)) {
				throw new ConverterException("包含运算符比较值集合值必须为文本");
			}
		}
	}
	...
	public void addCondition(MySqlSelectQueryBlock queryBlock, ColumnType columnType, String columnCode, Object value) {
		if (value instanceof String) {
			queryBlock.addCondition(columnCode + " in ('" + value.toString() + "')");
			return;
		}
		...
		queryBlock.addCondition(columnCode + " in ('" + condition + "')");
	}
