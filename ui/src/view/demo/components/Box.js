...
const boxSource = {
	...
	endDrag(props, monitor) {
		...
		if (dropResult) {
            ...
            // this.props.handleCondition(item);
		}
	},
}
...
class Box extends React.Component {
	...
	static propTypes = {
		...
		orderLable: PropTypes.string.isRequired,
	...
	render() {
		...
		const { name, handleCondition, type, orderBy, orderLable, dataType } = this.props;
		...
		return connectDragSource && connectDragSource(
            <div style={{ ...style, opacity }}>
				{ 
					dataType === 'string' ? <div className="fx fx-y-center">
						<i className="icon-str"></i>
						<span>{name}</span>
					</div> : dataType === 'number' ? <div className="fx fx-y-center">
						<i className="icon-no"></i>
						<span>{name}</span>
					</div> : <div className="fx fx-y-center">
