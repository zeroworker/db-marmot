...
class LatitudeR extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedKey: '',
      ...
      return (
        <TreeNode
          ...
          title={
            <span>
                            {value.title}
              <Icon type="down" onClick={() => this.handleMore(value)}/>
                            <Icon type="block"/>
                            <Icon type="delete" onClick={() => this.handleDelete(value)}/>
                        </span>
          }
          key={value.key}
        >
...
export default DropTarget('latitude', spec, collect)(LatitudeR)
